  ...
  async (filters: FilterQuery[], thunkApi) => {
    ...
    try {
      const result = await sl
        .get<GetAllEmployeesUseCase>(ServiceKeys.GetAllEmployeesUseCase)
        .call(filters);
...
export const createEmployee = createAsyncThunk(
  ...
  async (data: EmployeeInterface, thunkApi) => {
    ...
    try {
      ...
      const result = await sl
        .get<CreateEmployeeUseCase>(ServiceKeys.CreateEmployeeUseCase)
        .call(new CreateEmployeeUseCaseParameters(data))
        .then(() => {
...
export const updateEmployee = createAsyncThunk(
  ...
  async (data: EmployeeInterface, thunkApi) => {
    ...
    try {
      const result = await sl
        .get<UpdateEmployeeUseCase>(ServiceKeys.UpdateEmployeeUseCase)
        .call(new UpdateEmployeeUseCaseParameters(data))
        .then(() => {
          dispatch(getEmployeeList([]));
        });
      console.log("Result:", result);
...
export const deleteEmployee = createAsyncThunk(
  ...
  async (id: string, thunkApi) => {
    ...
    try {
      const result = await sl
        .get<DeleteEmployeeUseCase>(ServiceKeys.DeleteEmployeeUseCase)
        .call(new DeleteEmployeeUseCaseParameters(id))
        .then(() => {
          dispatch(getEmployeeList([]));
        });
      console.log("Result:", result);
...
export const getEmployeeDetails = createAsyncThunk(
  ...
  async (id: string, thunkApi) => {
    ...
    try {
      const result = await sl
        .get<GetEmployeeByIdUseCase>(ServiceKeys.GetEmployeeByIdUseCase)
        .call(new GetEmployeeByIdUseCaseParameters(id));
      console.log("Result:", result);
      return result;
    } catch (error) {
      console.log("Error:", error);
