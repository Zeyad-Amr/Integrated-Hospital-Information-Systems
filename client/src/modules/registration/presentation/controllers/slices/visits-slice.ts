...
const visitSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    //* Create Transfer
    builder.addCase(createVisitTransfer.pending, (state, _action) => {
      state.loading = true;
      state.error = "";
    });
    builder.addCase(createVisitTransfer.fulfilled, (state, _action) => {
      state.loading = false;
      state.currentVisit = initialState.currentVisit;
      state.visits = PaginatedListModel.resetPaginatedList(state.visits);
      state.error = "";
      AlertService.showAlert("تم نقل المريض بنجاح", "success");
    });
    builder.addCase(createVisitTransfer.rejected, (state, action) => {
      state.loading = false;
      state.error = (action.payload as ErrorResponse).message;
      AlertService.showAlert(`${state.error}`, "error");
    });
...
export const { setLoading } = visitSlice.actions;
