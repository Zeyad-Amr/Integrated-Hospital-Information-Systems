...
import { getPermissionsList } from '@/modules/management/presentation/controllers/thunks/permissions-thunks';
...
import { SubDepartmentsState, RoomState, SpecializationState } from '@/modules/management/presentation/controllers/types';
...
const EndExaminationForm = ({ visitId, discharge }: { visitId: string, discharge: boolean }) => {
    ...
    function filterAndTransform (items: any) {
        console.log(items);
        return items?.filter((item: SubDepartmentInterface) => item.departmentId === 9 || item.departmentId === 10)
            .map((item: SubDepartmentInterface) => ({ id: item.id, value: item.name }));
    }
    ...
    return (
        ...
        >
            {({
                ...
                return (
                    <Box component="form" onSubmit={handleSubmit} noValidate sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                        <CustomSelectField
                            name="transfer"
                            noMargin
                            height="2.5rem"
                            ...
                            options={discharge ? dischargeOptions :  filterAndTransform(subDepartmentsState.subDepartments.items)}
                            width='14rem'
                        />
                        <PrimaryButton
