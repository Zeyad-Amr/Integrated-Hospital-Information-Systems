    ...
    useEffect(() => {
        ...
        apiData.forEach((item) => {
            ...
            newTableData.push({
                ...
                watingTime: <Box color={getColorBasedOnTime(item?.createdAt)} fontWeight={600}>
    ...
    return (
        ...
        >
            <CustomBasicTable
                ...
                rowProps={{
                    onDoubleClick: (event) => {
                        refPatientData.current = event.currentTarget.getAttribute('data-row') && JSON.parse(event.currentTarget.getAttribute('data-row') as string);
                        setShawDialog(true);
                    }
                }}
            />
            <ErAreaForm openDialog={showDialog} setOpenDialog={setShawDialog} patientData={refPatientData.current} />
