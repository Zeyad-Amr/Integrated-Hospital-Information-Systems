      ...
      switch (filter.rule) {
        ...
        case FilterRule.EQUALS:
          if (['createdAt', 'updatedAt'].includes(target)) {
            ...
            query = {
              AND: [
                { [target]: { gte: startOfTheDay.toISOString() } },
                { [target]: { lte: endOfTheDay.toISOString() } },
              ],
            };
          } else {
            query = {
        ...
        case FilterRule.GREATER_THAN_OR_EQUALS:
          if (['createdAt', 'updatedAt'].includes(target)) {
            ...
            query = {
              [target]: {
                [filter.rule === FilterRule.GREATER_THAN ? 'gt' : 'gte']:
        ...
        case FilterRule.LESS_THAN_OR_EQUALS:
          if (['createdAt', 'updatedAt'].includes(target)) {
            ...
            query = {
              [target]: {
                [filter.rule === FilterRule.LESS_THAN ? 'lt' : 'lte']:
        ...
        case FilterRule.STARTS_WITH:
          query = {
            [target]: { startsWith: filter.value, mode: 'insensitive' },
          };
          break;
      }
